...
% Override metropolis's default dark-teal with the same deep blue
% used for the GRPO bar in Figure 1.
\definecolor{accent}{HTML}{08519c}       % deep blue (used for GRPO bar in Fig 1)
\definecolor{accentMid}{HTML}{2171b5}    % mid blue
\definecolor{accentLight}{HTML}{6baed6}  % light blue (SFT bar in Fig 1)
\definecolor{baselineGrey}{HTML}{bdbdbd}

% Re-bind metropolis's internal theme colours to our blue palette.
\colorlet{mDarkTeal}{accent}
\colorlet{mLightBrown}{accentMid}

% Enforce the rebinding everywhere metropolis uses these colours.
\setbeamercolor{normal text}{fg=black, bg=white}
\setbeamercolor{alerted text}{fg=accent}
\setbeamercolor{progress bar}{fg=accent}
\setbeamercolor{title separator}{fg=accent}
\setbeamercolor{frametitle}{bg=accent, fg=white}
\setbeamercolor{frametitle right}{bg=accent, fg=white}
\setbeamercolor{block title}{bg=accent, fg=white}
\setbeamercolor{block title alerted}{bg=accent!85, fg=white}
...
% standout (closing) slide -- metropolis pulls its bg from palette primary
\setbeamercolor{palette primary}{bg=accent, fg=white}
\setbeamercolor{standout}{bg=accent, fg=white}
...
% ============================================================
\begin{frame}{What is a research agent --- and what's the question?}

\textbf{Research agent.} LM that issues real tool calls
(search, read), reasons over the results, over several rounds.
ReAct, Toolformer; Perplexity, Deep Research at the product level.

\vspace{0.4em}

\textbf{Agentic RL = a paradigm shift, not just a recipe} (Zhang
et al., arXiv 2509.02547, TMLR 2026; 500+ papers):
\begin{center}
\begin{tikzpicture}[node distance=1.4em, font=\footnotesize]
\node[draw, rounded corners, fill=baselineGrey!30, minimum height=0.85cm, minimum width=4.5cm, align=center]
  (mdp) {classical LLM-RL / RLHF\\\emph{degenerate single-step MDP}};
\node[draw, rounded corners, fill=accent, text=white, minimum height=0.85cm, minimum width=4.5cm, align=center, right=1.5em of mdp]
  (pomdp) {\textbf{Agentic RL}\\\emph{temporally-extended POMDP}};
\draw[-{Stealth}, thick, accent] (mdp) -- (pomdp);
\end{tikzpicture}
\end{center}
Tool-using research agents are one instance: multi-turn rollouts,
real observations between steps, trajectory-level reward.

\vspace{0.4em}

\textbf{Our question.}\;
What does this recipe buy you on a \emph{0.5B} open policy --- once
you compare it head-to-head with a properly-tuned SFT baseline?

% Speaker notes
% -------------
% "Research agents are the search-then-reason loop in Perplexity,
% Deep Research, ReAct. The bigger picture is the paradigm shift the
% recent Zhang et al. survey calls 'Agentic RL': moving from the
% degenerate single-step MDPs of RLHF -- one prompt, one completion,
% one reward -- to the temporally-extended POMDPs of multi-turn
% agents with real environmental observations. The survey covers
% 500+ papers in this space.
%
% Our experiment is a narrow point in that landscape: what does the
% recipe buy you on a 0.5B model, once you compare it head-to-head
% with a carefully-configured SFT baseline? That comparison is where
% the talk is going."
\end{frame}
