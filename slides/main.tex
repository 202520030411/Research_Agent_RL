...
over several rounds. ReAct, Toolformer; deployed at scale in
Perplexity and Deep Research.

\vspace{0.5em}

\textbf{Training-with-real-tools is the 2025 frontier.}
Search-R1, R1-Searcher(++), RAGEN/StarPO, WebAgent-R1 all do
multi-turn RL where the \emph{real} retrieval / web / code tool
is dispatched between rollout steps, on top of GRPO-style
within-group advantage. Reported gains: $+20$--$41$ EM on
3--7B-parameter open models against RAG baselines.

\vspace{0.5em}

\textbf{Our question, narrower.}\quad
What does this recipe buy you on a \emph{0.5B} open policy that
fits on a free-tier T4 --- and what does it look like once you
strip away inference-time confounds and put it head-to-head with
a properly-configured SFT baseline?

% Speaker notes
% -------------
% "Research agents are the search-then-reason loop in Perplexity,
% Deep Research, ReAct. Training them with RL where the REAL tool is
% executed between rollout steps -- as opposed to having the model
% hallucinate the tool output -- is a very active line of work in 2025:
% Search-R1, R1-Searcher, RAGEN, WebAgent-R1, all real-tool multi-turn
% RL on top of GRPO-style advantage. They report 20-40% gains at 3-7B
% scale.
%
% Our question is narrower. What does this recipe actually buy you on
% a 0.5B model on a free T4? And -- crucially -- once you compare it
% head-to-head with a CAREFULLY CONFIGURED SFT baseline, not just any
% baseline. That's where the talk is going."
\end{frame}
